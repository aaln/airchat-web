...
export const GetMessages = cache(async ({ token, recordingIds }) => {
...
export const GetMessageThreadDetailsByReference = cache(
  async ({ token, recordingId }) => {
    const messageClient = new MessageAPIClient(
      airchatHostUrl,
      grpc.credentials.createSsl()
    );
    ...
    try {
      const response = await new Promise((resolve, reject) => {
        messageClient.getMessageThreadDetailsByReference(
          threadDetailsReq,
          metadata,
          (error, response) => {
            if (error) {
