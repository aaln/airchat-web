...
export default function TranscriptionWrapper({ message, charLimit }) {
  ...
  const renderTranscription = (message, displayText) => {
    ...
    return (
      <>
        {isExpanded ? visibleSentenceGroups.map((group, groupIndex) => (
  ...
  return (
    <div 
      className={`relative transition-opacity duration-500 ease-in-out ${isExpanded || !isTruncated ? 'opacity-100' : 'opacity-90 hover:opacity-100'}`}
    >
