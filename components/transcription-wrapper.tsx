...
export default function TranscriptionWrapper({ message }) {
  ...
  const isTruncated = sentenceGroups.length > 3 && !isExpanded;

  const renderTranscription = (message) => {
    const visibleSentenceGroups = isExpanded || sentenceGroups.length <= 3 ? sentenceGroups : sentenceGroups.slice(0, 3);
      ...
      <>
        {visibleSentenceGroups.map((group, groupIndex) => (
          <React.Fragment key={groupIndex}>
            ...
            {groupIndex < visibleSentenceGroups.length - 1 && <div>&nbsp;</div>}
          </React.Fragment>
        ))}
  ...
  return (
    <div 
      className={`relative transition-opacity duration-500 ease-in-out ${isExpanded || sentenceGroups.length <= 3 ? 'opacity-100' : 'opacity-90 hover:opacity-100'}`}
    >
      {renderTranscription(message)}
      {isTruncated && 
      
      <div className="absolute text-blue-600 cursor-pointer w-56 py-1 rounded-full flex justify-center items-center bg-white -bottom-9 left-1/2 transform -translate-x-1/2"
        onClick={(e) => {
            e.preventDefault();
            setIsExpanded(true);
        }}
