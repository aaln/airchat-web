      ...
      if (searchQuery !== '') {
        try {
          ...
          const response = await fetch(
            `api/search/enhanced?token=${accessToken}&query=${encodeURIComponent(
              searchQuery
            )}`
          );
                ...
                return (
                  ...
                  >
                    ...
                    <div className='flex mt-2'>
                      {message?.mentionedUsersList?.map(
                        (mentionedUser, userIndex) => (
                          <img
                            key={userIndex}
                            src={`${airchatCDNUrl}/${mentionedUser?.avatar}`}
