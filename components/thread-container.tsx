  ...
  return (
    <>
      <div
        ...
        className='my-8'
        style={{
          marginLeft: `${level * (window.innerWidth < 768 ? 10 : 30)}px`
        }}
      >
...
export default function ThreadContainer({ threadId }) {
    ...
    const fetchData = async (accessToken) => {
      try {
        ...
        const response = await fetch(
          `/api/thread/${threadId}?token=${accessToken}`
        );
        const results = await response.json();
        setFullThread(results);

        const parentMessageIds = Object.keys(results?.messagesTree);
        const childMessageIds = Object.values(results?.messagesTree).reduce<
          string[]
        >((acc, val) => acc.concat(val as string[]), []);
        const originId = parentMessageIds.filter(
          (parentId) => !childMessageIds.includes(parentId)
        );
  ...
  return (
    <>
      <Header />
      {fullThread?.messagesCache && (
        <h2 className='w-full flex items-center justify-center p-4 text-xl'>
          {Object.keys(fullThread?.messagesCache).length} Messages
        </h2>
      )}
