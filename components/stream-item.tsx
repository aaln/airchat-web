...
export function StreamItem({ item }) {
  ...
  return (
    <div
      ref={itemRef}
      className='relative flex flex-col gap-14 pt-6 w-full justify-center max-w-2xl'
    >
      {item?.referenceMessage && (
        <RenderMessage
          item={item}
          message={item?.referenceMessage || item?.firstMessage}
        />
      )}
      {threadDetails?.length > 1 && (
        <div className='ml-16 flex flex-col gap-2'>
          <div>{threadDetails?.length} Responses</div>
          <div className='flex flex-row flex-wrap gap-2 items-center'>
            {threadDetails?.map((detail, index) => (
              ...
              >
                <AvatarImage
                  src={`${airchatCDNUrl}/${detail?.firstMessage?.fromUser?.avatar}`}
                  alt={detail?.firstMessage?.fromUser?.name}
                />
                <AvatarFallback delayMs={600}>
                  {detail?.firstMessage?.fromUser?.name.charAt(0)}
                </AvatarFallback>
              </Avatar>
            ))}
          </div>
      ...
      {selectedDetails && (
        <RenderMessage item={item} message={selectedDetails?.firstMessage} />
      )}
      {!selectedDetails &&
        item?.messagesList?.map((message, index) => (
          <RenderMessage key={index} item={item} message={message} />
        ))}
      <div className='w-full h-2 bg-[#F7F8FA]'></div>
      {item.type !== 1 && (
        <div className='absolute left-[20px] z-0 h-full w-0.5 bg-gray-300'></div>
      )}
    </div>
  );
}
