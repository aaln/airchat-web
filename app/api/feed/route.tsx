...
// @ts-ignore
import { ChannelSearchRequest, MessageSearchRequest } from '@/airchat/search/v2/enhanced_search_api_pb';
...
export async function GET(request: Request) {
    ...
    const enhancedSearchClient = new EnhancedSearchAPIClient(airchatHostUrl, grpc.credentials.createSsl()); 
    ...
    return Promise.all([
        new Promise((resolve, reject) => {
            enhancedSearchClient.channelSearch(channelSearchReq, metadata, (error, response) => {
                if (error) {
        ...
        new Promise((resolve, reject) => {
            enhancedSearchClient.messageSearch(searchReq, metadata, (error, response) => {
                if (error) {
