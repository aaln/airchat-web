...
export async function GET(request: Request) {
    ...
    const messageClient = new MessageInternalAPIClient(airchatHostUrl, grpc.credentials.createSsl());
    const metadata = new grpc.Metadata();
    metadata.add('authorization', `Bearer ${token}`);

    const hydratedMessagesReq = new GetHydratedMessagesRequest();
    hydratedMessagesReq.setRecordingIdList(["416914d3-c85c-4b07-b153-41447a577b1b"]);
    return new Promise<Response>((resolve, reject) => {
        messageClient.getHydratedMessages(hydratedMessagesReq, metadata, (error, response) => {
            if (error) {
