...
export async function GET(request: Request) {
    ...
    return new Promise<Response>((resolve, reject) => {
        client.refresh(refreshRequest, metadata, (error: grpc.ServiceError, response: any) => {
            if (error) {
                console.error('Error:', error);
                reject(new Response(JSON.stringify({ error: error.message }), { status: 500, statusText: "Internal Server Error" }));
            } else {
                resolve(new Response(JSON.stringify(response.toObject())));
            }
        });
    });
}